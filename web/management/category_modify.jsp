...
<head lang="zh">
    ...
    <link href="../management-css/category_modify.css" rel="stylesheet" type="text/css">
    ...
    <script>
        $(function () {
            $('#img').click(function () {
                var old = $('#old').html();
                var news = $('#new').val();
                var ne = document.getElementById('main_center_right2_5');
                var n = ne.options[ne.selectedIndex].text;
                $.ajax({
                    type:'POST',
                    url:'update.do',
                    data:{level:'second',oldname:old,newname:news,newclass:n},
                    cache:false,
                    success:function(d){
                        alert(d);
...
<main>
    ...
    <div class="main_center_left">
        <p><img src="../management-css/images/ai.png"><strong>修改二级分类</strong></p>
        <hr class="line">
        <!--中间修改部分-->
        <div class="min_center_right">
            修改前的旧名称(*):<span id="old"><%=request.getParameter("twotype")%></span>
        </div>
        <div class="main_center_right2">
            <div class="main_center_right2_1">修改后的新名称(*):</div>
            <input type="text" id="new" size="10">
        </div>
        <div class="main_center_right2_2">修改前所属一级分类：</div>
        <div class="main_center_right2_3" >
           <span id="main_center_right2_3"><%=request.getParameter("twoty")%></span>
        </div>
        <div class="main_center_right2_4">修改后所属一级分类：</div>
        <select class="main_center_right2_5" id="main_center_right2_5">
            <%
                MyDbUtil mn = new MyDbUtil();
                List<Map<String,Object>>lis=mn.query("oll_type1","name","where 1=1","","");
                for (Map<String,Object> m:lis){
            %>
            <option><%=m.get("name")%></option>
            <% }%>
        </select>
        <div class="main_center_right3"><input type="image" id="img" src="../management-css/images/submit.gif"></div>
    </div>
</main>
<footer>
    <div>
